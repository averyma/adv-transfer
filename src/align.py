...
def align_feature_space_mixed(train_loader, list_trainable, list_witness_model, criterion_kd, criterion_cls, optimizer, lr_scheduler, scaler, epoch, device, args, is_main_task):
    batch_time = AverageMeter('Time', ':6.3f')
    data_time = AverageMeter('Data', ':6.3f')
    losses = AverageMeter('Loss', ':.4e')
    top1 = AverageMeter('Acc@1', ':6.2f')
    ...
    # switch to train mode
    source_model.train()
    witness_model = list_witness_model[0]
    witness_model.eval()
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        if args.source_arch.startswith('vit'):
        ...
        feat_s = source_projection(features['feat_s'].view(images.size(0), -1))
        ...
        loss_align = loss_kd + DistillKL(args.kl_temp, args.kl_reduction)(p_s, p_w.unsqueeze(0))
