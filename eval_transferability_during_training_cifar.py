...
def main():
    ...
    # create a list of target models:
    list_target_model = []
    for target_arch in ['preactresnet18', 'preactresnet50', 'vgg19', 'vit_small']:
        args.arch = target_arch
        target_model_dir = root_dir + model_ckpt[args.dataset][args.arch]
        target_model_ckpt = target_model_dir + '5/model/best_model.pt'
        target_model = get_model(args, device)
        target_model.load_state_dict(torch.load(target_model_ckpt))
        print('target model loaded from {}'.format(target_model_ckpt))
    ...
    for _epoch in range(ckpt_epoch, args.epoch+1):
        ...
        transfer_log = test_transfer_multiple_target(test_loader, args, model, list_target_model, device)
        for i, target_arch in enumerate(['preactresnet18', 'preactresnet50', 'vgg19', 'vit_small']):
            print('transfer to {}: {}'.format(target_arch, transfer_log[0][i]))
            logger.add_scalar("transfer/{}".format(target_arch), transfer_log[0][i], _epoch)
